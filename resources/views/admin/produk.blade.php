								...
								@foreach ($produk as $item)
									<tr>
										...
										@if ($item->harga_diskon)
											<td>
												<span class="me-3">Rp. {{ number_format($item->harga_diskon) }}</span>
												<small class="text-black-50" style="text-decoration-line: line-through">Rp.
													{{ number_format($item->harga) }}</small>
												<span class="badge rounded-pill bg-danger">{{ $item->diskon }} %</span>
											</td>
										@else
											...
											<div class="row">
												<div class="col-12">
													<button class="btn btn-xs btn-info" data-bs-toggle="modal"
														data-bs-target="#modalDetail{{ $item->id }}"><i class="bx bx-info-circle"></i> Detail</button>
													<button class="btn btn-xs btn-success" data-bs-toggle="modal"
														data-bs-target="#modalFoto{{ $item->id }}"><i class="bx bxs-photo-album"></i> Tambah Foto</button>
													<a class="btn btn-xs btn-warning" href="{{ route('produk.edit', $item->id) }}"><i
															class="bx bx-edit-alt"></i> Edit</a>
													<button class="btn btn-xs btn-danger" data-bs-toggle="modal"
														data-bs-target="#modalDelete{{ $item->id }}"><i class="bx bx-trash-alt"></i> Delete</button>
												</div>
											</div>
										</td>
									</tr>
					...
					<div class="modal-body">
						<div class="row">
							<div class="col-md-4 mb-3">
								<img class="img-thumbnail mx-auto shadow" src="{{ asset('storage/' . $item->image) }}" width="200"
									alt="Foto {{ $item->name }}" class="img-thumbnail">
							</div>
							<div class="col-md-8">
